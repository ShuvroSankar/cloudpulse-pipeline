...
WITH metrics AS (
  SELECT
    ...
    -- Pre-calculate Wasted Spend directly at the log row level (5xx Server Errors)
    CASE 
    ...
    -- Compute service p95 response time across the entire partition
    PERCENTILE_CONT(response_ms, 0.95) OVER (
      PARTITION BY service_name
    ) AS service_p95_ms,

    -- Sliding 50-request window for rolling statistical mean and standard deviation
...
SELECT
  ...
  ROUND((rolling_50_req_cost_usd / 50.0) * 1000.0, 4) AS rolling_unit_cost_per_1k_req_usd,
  ...
  -- Service Health Status for Conditional Formatting
  CASE 
